...
# Function to calculate max-min classes for the specific indicator
calculate_max_min_classes <- function(data, indicator_name) {
  data <- data %>%
    select(Year, Country, directionality, !!sym(indicator_name)) %>%  # Step 1: Select indicator
    filter(!is.na(!!sym(indicator_name))) %>%  # Step 2: Remove NA values
    mutate(
      min_value = min(!!sym(indicator_name)),
      max_value = max(!!sym(indicator_name)),
      range_value = max_value - min_value,
      class_interval = range_value / 6,
      class = case_when(
        directionality == "pos" & !!sym(indicator_name) <= min_value + class_interval ~ 1,
        directionality == "pos" & !!sym(indicator_name) <= min_value + 2 * class_interval ~ 2,
        directionality == "pos" & !!sym(indicator_name) <= min_value + 3 * class_interval ~ 3,
        directionality == "pos" & !!sym(indicator_name) <= min_value + 4 * class_interval ~ 4,
        directionality == "pos" & !!sym(indicator_name) <= min_value + 5 * class_interval ~ 5,
        directionality == "pos" ~ 6,
        directionality == "neg" & !!sym(indicator_name) >= max_value - class_interval ~ 1,
        directionality == "neg" & !!sym(indicator_name) >= max_value - 2 * class_interval ~ 2,
        directionality == "neg" & !!sym(indicator_name) >= max_value - 3 * class_interval ~ 3,
        directionality == "neg" & !!sym(indicator_name) >= max_value - 4 * class_interval ~ 4,
        directionality == "neg" & !!sym(indicator_name) >= max_value - 5 * class_interval ~ 5,
        directionality == "neg" ~ 6,
        TRUE ~ NA_integer_
      )
    )
...
plot_spider_chart <- function(countries, years, indicators) {
  # Calculate classes for each indicator dataset
  indicator_classes <- map(indicators, ~calculate_max_min_classes(reshaped_df, .x))
  ...
  for (country in countries) {
    for (year in years) {
      country_classes <- data.frame(
        Category = indicators,
        Class = sapply(1:length(indicators), function(i) {
          classes <- indicator_classes[[i]]$class[indicator_classes[[i]]$Country == country & indicator_classes[[i]]$Year == year]
          if (length(classes) == 0) {
            return(NA)
          } else {
            return(classes)
      ...
      if (!any(is.na(country_classes$Class))) {
        spider_chart <- spider_chart %>%
          add_trace(r = country_classes$Class, 
                    theta = country_classes$Category,
  ...
  spider_chart <- spider_chart %>%
    layout(
      polar = list(radialaxis = list(visible = TRUE, range = c(1, 6), tickmode = 'array', tickvals = 1:6, ticktext = 1:6)),
      showlegend = TRUE,
